...
use crate::{
    constants::CompressionCodec, footer::QfSectionEntryV1, postscript::QfSectionSpecV1, QfError,
};
...
///
/// Unknown codec values are reported as [`QfError::BadSection`].
...
fn payload_from_spec<'a>(
    ...
    let codec = CompressionCodec::from_u8(compression)
        .ok_or_else(|| QfError::BadSection(format!("unknown compression codec {compression}")))?;
    match codec {
...
fn payload_raw_bytes<'a>(
    ...
    let end = offset.checked_add(length).ok_or(QfError::ArithOverflow)?;
...
fn lz4_decompress(raw: &[u8], expected_len: u64) -> Result<Vec<u8>, QfError> {
    let expected = usize::try_from(expected_len).map_err(|_| QfError::ArithOverflow)?;
    lz4_flex::block::decompress(raw, expected)
        .map_err(|e| QfError::BadSection(format!("LZ4 decompression failed: {e}")))
}

#[cfg(not(feature = "compression-lz4"))]
fn lz4_decompress(_raw: &[u8], _expected_len: u64) -> Result<Vec<u8>, QfError> {
...
fn zstd_decompress(raw: &[u8], expected_len: u64) -> Result<Vec<u8>, QfError> {
    use std::io::Read;
    let expected = usize::try_from(expected_len).map_err(|_| QfError::ArithOverflow)?;
    let mut decoder = ruzstd::StreamingDecoder::new(raw)
        .map_err(|e| QfError::BadSection(format!("Zstd decoder init failed: {e}")))?;
    let mut out = Vec::with_capacity(expected);
    decoder
        .read_to_end(&mut out)
        .map_err(|e| QfError::BadSection(format!("Zstd decompression failed: {e}")))?;
    if out.len() != expected {
...
fn zstd_decompress(_raw: &[u8], _expected_len: u64) -> Result<Vec<u8>, QfError> {
    Err(QfError::UnsupportedEncoding(
...
mod tests {
    ...
    use crate::postscript::QfSectionSpecV1;
    ...
    fn lz4_round_trip_decompresses_payload() {
        let payload = b"Quay Format reference implementation showcase payload payload payload";
