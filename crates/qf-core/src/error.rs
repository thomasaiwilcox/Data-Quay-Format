    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            QfError::UnknownRequiredFeature(bits) => {
                write!(f, "QF_E_UNKNOWN_REQUIRED_FEATURE: unknown required feature bits 0x{bits:016x}")
            }
            QfError::ChecksumMismatch => write!(f, "QF_E_CHECKSUM_MISMATCH: CRC32C mismatch"),
            QfError::DigestMismatch => write!(f, "QF_E_DIGEST_MISMATCH: cryptographic digest mismatch"),
            QfError::OffsetRange => write!(f, "QF_E_OFFSET_RANGE: offset or length out of file bounds"),
            QfError::ArithOverflow => write!(f, "QF_E_ARITH_OVERFLOW: arithmetic overflow in offset/count"),
            ...
            QfError::BadLogicalPhysicalPair => {
                write!(f, "QF_E_BAD_LOGICAL_PHYSICAL_PAIR: logical type incompatible with physical kind")
            }
            QfError::DictMiss => write!(f, "QF_E_DICT_MISS: FileCode missing from dictionary"),
            QfError::BadFileCode => write!(f, "QF_E_BAD_FILECODE: FileCode outside dictionary range"),
            QfError::BadNumCode => write!(f, "QF_E_BAD_NUMCODE: NumCode invalid for declared logical type"),
            ...
            QfError::BadExtension => write!(f, "QF_E_BAD_EXTENSION: extension invalid or unsupported"),
            QfError::BadEngineProfile => {
                write!(f, "QF_E_BAD_ENGINE_PROFILE: engine profile invalid or unsupported")
            }
            QfError::ExecutionCodeMap => write!(f, "QF_E_EXECUTION_CODE_MAP: execution code mapping failed"),
            ...
            QfError::NotSelfContained => write!(f, "QF_E_NOT_SELF_CONTAINED: QF-O chain not self-contained"),
            ...
            QfError::RedactionPolicy => write!(f, "QF_E_REDACTION_POLICY: redacted value cannot be surfaced"),
            QfError::SidecarStale => write!(f, "QF_E_SIDECAR_STALE: QFX/QFM sidecar does not match QF file"),
