//! Quay Format (QF) v1.0 — Stable row references (Spec §54).
//!
//! Each row in a QF-T table can be addressed by the Spec §54.1
//! `QfTableRowRefV1` tuple: `(table_id, segment_id, morsel_id,
//! row_in_morsel)`.
...
impl RowRef {
    /// Encoded byte length of `QfTableRowRefV1` (Spec §54.1).
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, QfError> {
