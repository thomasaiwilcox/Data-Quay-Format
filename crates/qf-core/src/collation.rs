//! Quay Format (QF) v1.0 — Collation registry (Spec §22).
//!
//! Every QF v1 reader MUST recognise the six minimum collations defined by the
...
impl CollationKind {
    ...
    pub fn compare(self, lhs: &[u8], rhs: &[u8]) -> Result<Ordering3, QfError> {
        match self {
            CollationKind::None => {
...
fn check_same_len(lhs: &[u8], rhs: &[u8]) -> Result<(), QfError> {
...
fn unsigned_numeric(lhs: &[u8], rhs: &[u8]) -> Result<Ordering3, QfError> {
    check_same_len(lhs, rhs)?;
    Ok(read_unsigned(lhs).cmp(&read_unsigned(rhs)).into())
}

fn signed_numeric(lhs: &[u8], rhs: &[u8]) -> Result<Ordering3, QfError> {
    check_same_len(lhs, rhs)?;
...
mod tests {
    ...
    fn spec_22_none_supports_only_equality() {
        ...
        assert!(matches!(none.compare(b"a", b"b"), Err(QfError::BadStats)));
    }
