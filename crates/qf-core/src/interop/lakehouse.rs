...
//!
//! Optional descriptive metadata that integrates a QF file into a lakehouse
//! catalog (Iceberg, Delta, Hudi, …). Spec §50.6 makes hints **non-
//! authoritative**: they MUST never override QF's own structural semantics.
...
fn read_str(bytes: &[u8], pos: &mut usize) -> Result<String, QfError> {
    if *pos + 2 > bytes.len() {
        return Err(QfError::BufferTooShort);
    }
    ...
    *pos += 2;
    ...
    let s = std::str::from_utf8(&bytes[*pos..end])
        .map_err(|_| QfError::BadSection("lakehouse hint not UTF-8".into()))?
        .to_string();
    *pos = end;
    Ok(s)
}
