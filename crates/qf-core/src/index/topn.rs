...
impl TopNSummary {
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self, QfError> {
        ...
        let direction = TopNDirection::from_u8(header[16]).ok_or(QfError::BadIndex)?;
...
mod tests {
    ...
    fn rejects_bad_direction() {
        ...
        assert_eq!(TopNSummary::parse(&bytes), Err(QfError::BadIndex));
    }
    ...
    fn rejects_checksum_mismatch() {
        ...
        let mut bytes = summary.serialize_header();
        bytes[36] ^= 0xff;
        assert_eq!(TopNSummary::parse(&bytes), Err(QfError::ChecksumMismatch));
    }
}
