...
impl ExactSetIndexHeaderV1 {
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self, QfError> {
        ...
        let granularity = ExactSetGranularity::from_u8(bytes[8]).ok_or(QfError::BadIndex)?;
        let key_kind = ExactSetKeyKind::from_u8(bytes[9]).ok_or(QfError::BadIndex)?;
        let representation = ExactSetRepresentation::from_u8(bytes[10]).ok_or(QfError::BadIndex)?;
        Ok(Self {
...
mod tests {
    ...
    fn checksum_mismatch_rejected() {
        ...
        bytes[32] ^= 0xff;
        assert_eq!(ExactSetIndex::parse(&bytes), Err(QfError::ChecksumMismatch));
    }
