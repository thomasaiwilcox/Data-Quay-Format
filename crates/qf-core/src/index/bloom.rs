...
impl BloomIndexHeaderV1 {
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self, QfError> {
        ...
        let granularity = BloomGranularity::from_u8(bytes[8]).ok_or(QfError::BadIndex)?;
        let hash_domain = BloomHashDomain::from_u8(bytes[9]).ok_or(QfError::BadIndex)?;
        let algorithm = BloomAlgorithm::from_u8(bytes[10]).ok_or(QfError::BadIndex)?;
        Ok(Self {
...
mod tests {
    ...
    fn rejects_zero_filter_count() {
        ...
        let mut bytes = header.serialize().to_vec();
        bytes.extend_from_slice(&bits);
        assert_eq!(BloomFilterIndex::parse(&bytes), Err(QfError::BadIndex));
    }
}
