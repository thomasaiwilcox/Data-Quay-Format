...
impl SortKeyEntryV1 {
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self, QfError> {
        ...
        let direction = SortDirection::from_u8(bytes[4])
            .ok_or_else(|| QfError::BadSection(format!("bad sort direction {}", bytes[4])))?;
        let null_order = NullOrder::from_u8(bytes[5])
            .ok_or_else(|| QfError::BadSection(format!("bad null order {}", bytes[5])))?;
...
mod tests {
    ...
    fn sort_key_entry_rejects_short_buffer() {
        let bytes = [0u8; 4];
        assert_eq!(SortKeyEntryV1::parse(&bytes), Err(QfError::BufferTooShort));
    }
