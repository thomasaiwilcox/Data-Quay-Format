//! Quay Format (QF) v1.0 — Column page index and page header (Spec §27).
//!
...
use crate::{checksum::crc32c, constants::QfEncodingKind, QfError};
...
impl PageEntry {
    ...
    pub fn verify_crc(&self, payload: &[u8]) -> Result<(), QfError> {
        let end = self
            .offset
...
mod tests {
    ...
    fn rejects_null_count_above_row_count() {
        ...
        assert_eq!(PageIndex::parse(&bytes), Err(QfError::PageCorrupt));
    }
