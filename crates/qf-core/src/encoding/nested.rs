...
pub fn validate_no_duplicate_keys(keys: &[Vec<u8>]) -> Result<(), QfError> {
...
mod tests {
    ...
    fn list_offsets_must_start_at_zero() {
        ...
        assert_eq!(l.validate(), Err(QfError::PageCorrupt));
    }
    ...
    fn struct_fields_must_share_row_count() {
        ...
        assert_eq!(s.validate(), Err(QfError::PageCorrupt));
    }

    #[test]
    fn map_duplicate_key_within_value_rejected() {
        let keys = vec![b"a".to_vec(), b"a".to_vec()];
        assert_eq!(validate_no_duplicate_keys(&keys), Err(QfError::PageCorrupt));
    }
