//! Quay Format (QF) v1.0 — Canonical value encoding (Spec §17).
//!
//! The canonical encoding is the byte-level identity of a value in QF. Two
...
use crate::{constants::ValueTag, wire, QfError};
...
impl<'a> CanonicalValue<'a> {
    ...
    pub fn encode(&self) -> Result<Vec<u8>, QfError> {
        match self {
...
fn encode_i64_width(width: u8, value: i128) -> Result<Vec<u8>, QfError> {
    let min = match width {
        1 => i8::MIN as i128,
        2 => i16::MIN as i128,
        4 => i32::MIN as i128,
...
fn encode_u64_width(width: u8, value: u128) -> Result<Vec<u8>, QfError> {
    let max = match width {
        1 => u8::MAX as u128,
        2 => u16::MAX as u128,
        4 => u32::MAX as u128,
...
fn encode_tagged(value: &CanonicalValue<'_>) -> Result<Vec<u8>, QfError> {
...
) -> Result<Vec<(Vec<u8>, Vec<u8>)>, QfError> {
...
pub fn canonicalize_list(elements: &[CanonicalValue<'_>]) -> Result<Vec<u8>, QfError> {
...
pub fn canonicalize_struct(fields: &[CanonicalField<'_>]) -> Result<Vec<u8>, QfError> {
