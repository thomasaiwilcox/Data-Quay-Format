//! Quay Format (QF) v1.0 — descriptive footer metadata JSON.
//!
...
use crate::{constants::METADATA_LEN_MAX, QfError};
...
pub fn validate(bytes: &[u8]) -> Result<(), QfError> {
    ...
    std::str::from_utf8(bytes).map_err(|_| {
        QfError::BadSection("Spec §15: metadata_json must be valid UTF-8".to_string())
    })?;
