//! `qf-bench` — Quay Format reference benchmark harness (Spec §78).
//!
...
use qf_core::{canonical::CanonicalValue, checksum, interop::arrow};
...
fn bench_arrow_inversion() {
    ...
    let qf_null = vec![0xA5u8; (row_count + 7) / 8];
    let iters = 16;
    let start = Instant::now();
    let mut sink = 0u8;
    for _ in 0..iters {
        let v = arrow::qf_null_to_arrow_validity(&qf_null, row_count)
