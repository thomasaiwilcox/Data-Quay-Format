 Quay Format Specification v1.0
A quay is where ships dock to unload cargo. Quay Format is where data lands ready to be queried.
Field	Value
Format Name	Quay Format
Acronym	QF
Primary Data File Magic	QYF1
Footer Magic	QYFF
Accelerator Sidecar Magic	QYX1
Dataset Manifest Magic	QYM1
Legacy Draft Magic	HQF1, reserved for pre-v1 Harbor drafts
Canonical Extension	.quay
Short Extension	.qf
Accelerator Sidecar Extension	.qfx
Dataset Manifest Extension	.qfm
MIME Type	application/vnd.quay-format
Version	1.0
...
This document defines Quay Format v1.0, hereafter QF.
QF defines the following profiles and companion artifacts:
QF-Core:
  Common immutable file structure, section directory, dictionary,
  logical/physical types, encoded arrays, checksums, validation,
  collation, canonical values, and extension rules.

QF-T:
  Engine-neutral table-scan profile.

QF-A:
  Archive acceleration profile for synopses, lookup indexes,
  composite pruning, manifests, and sidecar acceleration.

QF-E:
  Universal engine execution profile for mapping FileCodes into
  implementation-local ExecutionCodes.

QF-H:
  Harbor registration under QF-E. Defines Harbor leased-code execution:
  FileCode -> Harbor EngineCode.

QF-O:
  Object-temporal profile for Harbor-style object history, deltas,
  branches, CSNs, baselines, snapshots, tombstones, and trust chains.

QFX:
  Optional accelerator sidecar.

QFM:
  Optional dataset manifest.
A conforming QF reader MUST be able to validate and read QF files without QFX or QFM.
QFX and QFM are optional acceleration and planning artifacts. They MUST NOT change the logical meaning of the referenced QF files.
 
⸻
 
...
 
3. Purpose
QF is an immutable, queryable, encoded data format designed for:
- high-performance offline/archive table scans,
- Parquet/ORC/CSV/Arrow-to-QF conversion,
...
- Harbor leased-code execution through QF-H,
- optional object-temporal history through QF-O.
QF is not:
- a WAL,
...
QF should be positioned as:
A queryable offline/archive format with encoded arrays, proof-carrying
predicate metadata, optional accelerator sidecars, and direct support for
engine-local dictionary execution.
It should not be positioned as:
A universal Parquet replacement.
Recommended positioning:
Parquet / ORC:
  universal lakehouse interchange and mature analytical columnar storage.

QF:
  high-performance queryable archive and converted-table format for engines
  that can exploit encoded execution, rich pruning metadata, lookup indexes,
  aggregate synopses, optional sidecars, and direct dictionary/code-vector
  execution.
 
⸻
 
...
QF-Core	Core Format	All readers/writers	File layout, sections, dictionary, encodings, checksums, validation.
QF-T	Table Scan Profile	General engines	Engine-neutral columnar table scan profile.
QF-A	Archive Acceleration Profile	Archive/query engines	Synopses, lookup indexes, manifests, composite pruning, sidecars.
QF-E	Engine Execution Profile	All engines	Universal mapping from FileCodes to engine-local ExecutionCodes.
QF-H	Harbor Execution Registration	Harbor	Harbor leased-code implementation of QF-E.
QF-O	Object Temporal Profile	Harbor and temporal-object engines	Object history, deltas, branches, CSNs, trust chains.
 
⸻
 
6. Core Concepts
...
Rules:
- FileCode is local to exactly one QF file.
- FileCode equality is meaningful only within the same QF file.
...
QF-Core does not define the meaning of an ExecutionCode.
 
QF-E defines the universal mechanism for describing execution-code mappings.
 
QF-H defines Harbor’s implementation:
FileCode -> Harbor EngineCode
 
⸻
 
...
- not authoritative QF file data.
QF files MUST NOT persist Harbor EngineCodes as authoritative data.
 
⸻
 
6.4 NumCode
...
 
6.7 Morsel
A morsel is QF-T’s fundamental scan unit.
...
7. Core Invariants
7.1 QF is immutable
QF files are write-once-read-many.
...
Compaction, import, export, and conversion produce new QF files.
 
⸻
 
7.2 QF is engine-neutral at the core
QF-Core and QF-T MUST be readable without Harbor.
...
QF-H is Harbor-specific, but it is registered through the universal QF-E mechanism.
 
⸻
 
7.3 Engine profiles do not define logical truth
INVARIANT:
  Engine profiles accelerate or adapt execution; they do not define QF logical truth.
A QF file’s logical values are determined by:
- QF-Core,
- file dictionary,
- logical types,
- physical streams,
...
- mount state,
- dictionary arrays.
Engine profiles MUST NOT be required to recover the logical values of a QF-T file.
 
⸻
 
...
QF MAY include cryptographic digests.
...
struct QfHeaderV1 {
    ...
    primary_profile: u8,
    // 0=mixed/unknown
    // 1=QF-O object temporal
    // 2=QF-T table scan
    // 3=QF-A archive acceleration
    // 4=QF-E engine execution
    // 5=QF-H Harbor registered execution profile
...
Header rules:
- magic MUST be "QYF1" for QF v1.
- Readers MAY optionally accept "HQF1" as a legacy draft magic in compatibility mode.
...
Bit	Name	Meaning
0x0000_0000_0000_0001	FEATURE_OBJECT_PROFILE	File contains QF-O sections.
0x0000_0000_0000_0002	FEATURE_TABLE_PROFILE	File contains QF-T sections.
0x0000_0000_0000_0004	FEATURE_ARCHIVE_PROFILE	File contains QF-A sections.
0x0000_0000_0000_0008	FEATURE_ENGINE_PROFILE	File contains QF-E sections.
0x0000_0000_0000_0010	FEATURE_HARBOR_PROFILE	File contains QF-H Harbor-specific metadata.
...
 
12. Postscript
The final bytes of every QF file are:
[postscript bytes]
...
struct QfFooterHeaderV1 {
    footer_magic: [u8; 4],       // "QYFF"
...
// followed by:
//   QfSectionEntryV1[section_count]
//   metadata_json[metadata_len]
struct QfSectionEntryV1 {
    section_id: u32,
    section_kind: u16,
...
10	TABLE_CATALOG	QF-T	Table schemas.
11	TABLE_SEGMENT_INDEX	QF-T	Segment locators and row ranges.
12	TABLE_SEGMENT_DATA	QF-T	Table segment payloads.
13	COLUMN_DOMAIN	QF-T	Logical ordering for FileCodes.
14	ZONE_STATS	QF-T	Segment/morsel/page stats.
15	EXACT_SET_INDEX	QF-T/QF-A	Exact value-set indexes.
16	BLOOM_INDEX	QF-T/QF-A	Bloom filters.
17	INVERTED_MORSEL_INDEX	QF-T/QF-A	Value-to-morsel indexes.
18	LOOKUP_INDEX	QF-A	Point lookup indexes.
19	AGGREGATE_SYNOPSIS	QF-A	Counts, histograms, sketches.
20	COMPOSITE_ZONE_INDEX	QF-A	Multi-column pruning metadata.
21	TOPN_ZONE_SUMMARY	QF-A	Top/bottom zone summaries.
22	KERNEL_CAPABILITIES	QF-T/QF-A	Encoded-kernel capability metadata.
30	ENGINE_PROFILE_REGISTRY	QF-E	Registered engine execution profiles.
31	EXECUTION_CODE_DESCRIPTOR	QF-E	ExecutionCode description.
32	EXECUTION_SCOPE_DESCRIPTOR	QF-E	Execution scope metadata.
33	CODE_SPACE_DESCRIPTOR	QF-E	Code-space metadata.
34	ENGINE_MOUNT_POLICY	QF-E	Generic mount/execution mapping policy.
40	OBJECT_TYPE_CATALOG	QF-O	Object/property catalog.
41	TEMPORAL_SEGMENT_INDEX	QF-O	Temporal segment locators.
42	TEMPORAL_SEGMENT_DATA	QF-O	Temporal segment payloads.
43	TEMPORAL_BLOOM_INDEX	QF-O	Scope/branch/GOID/time bloom filters.
44	TRUST_MANIFEST	QF-O	Trust-chain metadata.
50	HARBOR_MOUNT_HINTS	QF-H	Harbor-specific lease/mount hints.
...
{
  "format_version": "1.0",
  "format_name": "Quay Format",
  "created_by": "quay-writer/<version>",
  "created_at_us": 0,
  "primary_profile": "QF-T",
  "source": {
    "format": "parquet",
    "schema_fingerprint": "",
    "conversion_policy": ""
...
 
20. Encoded Arrays
QF stores pages as encoded arrays.
...
QF-T MAY declare encoding kernel capabilities.
...
⸻
 
27. QF-T Column Directory and Pages
...
- Lookup indexes MAY be stored inside QF or in QFX.
...
⸻
 
37. QF-T Scan Semantics
A QF-T scan SHOULD proceed as:
...
5. Use QFM file-level pruning if available.
...
WHERE status = 'active'
Planner:
1. Resolve 'active' in QF dictionary.
...
⸻
 
38. QF-E Engine Execution Profile
QF-E allows an engine to map file-local physical values into implementation-local execution values without changing QF logical semantics.
Universal behaviour:
...
Rules:
- QF-E is optional.
- QF-E MUST NOT be required to decode QF-T logical values.
- QF-E MAY accelerate scans, output materialisation, joins, grouping, and dictionary execution.
- Unknown required QF-E profiles cause rejection only when the reader needs that profile.
- Unknown optional QF-E profiles MUST be ignored.
 
⸻
 
...
- Engine profiles MUST NOT change QF logical values.
...
Rules:
- QF logical nulls remain structural regardless of execution null-code policy.
- If an engine uses a runtime null code, that code is not a QF FileCode null sentinel.
...
- Code-space metadata MUST NOT be required to recover QF logical values.
...
⸻
 
44. QF-H Harbor Registration
QF-H is the Harbor registered implementation of QF-E.
Registration:
namespace:
  "io.harbor"
...
- HarborMountHints MUST NOT be required to decode QF-T values.
- Harbor readers MUST NOT treat on-disk FileCodes as Harbor EngineCodes.
44.2 Harbor Mount Steps
1. Validate QF structure and required sections.
...
8. Validate optional QFX/QFM if present.
...
- HarborMountCodeMap is not authoritative QF data.
- If missing or stale, it MUST be rebuilt.
- Harbor EngineCodes are not required for offline QF readability.
 
⸻
 
...
- Extensions MUST NOT change the meaning of QF-Core values unless the extension is required.
...
- Custom indexes may live in QF or QFX.
- Custom indexes MUST NOT change query results.
 
⸻
 
48. Engine-Neutral Mount / Read Protocol
A generic QF-T reader may expose data through:
...
9. Validate optional QFX/QFM if present.
10. Expose tables to the query planner.
 
⸻
 
49. Arrow Interop Profile
QF-T SHOULD support Arrow-compatible output.
Rules:
...
Arrow dictionary values:
  decoded from QF file dictionary.
Rules:
- Arrow interop MUST NOT require Harbor.
- Arrow interop MUST preserve QF logical type semantics.
- If a QF logical type cannot be represented exactly in Arrow, the reader MUST either expose an extension type or report a lossy conversion.
 
⸻
 
50. Lakehouse Integration Profile
QF is a file format, not a catalog.
QF files MAY be managed by:
- QFM native manifests,
- Iceberg,
- Delta,
- Hudi,
...
- Lakehouse hints MUST NOT override QF file semantics.
- Visibility/delete overlays are external in v1.
- QF files remain immutable.
Recommended usage:
Iceberg / Delta / Hudi:
  May use QF-T files as data files if the table engine has a QF reader.

QFM:
  Native lightweight QF dataset manifest for archive/object-store planning.

QFX:
  Optional acceleration sidecar for immutable QF files.
 
⸻
 
51. QF-T Parquet Conversion Profile
QF-T is intended as a high-performance conversion target for Parquet data.
The converter SHOULD rewrite data into QF-native scan layout, not copy Parquet’s physical layout.
51.1 Conversion Steps
1. Read Parquet schema and metadata.
2. Select target QF table schema.
3. Select table segment boundaries.
4. Select morsel row count.
5. Decode Parquet pages as needed.
6. Build QF file-local dictionary.
...
18. Encode pages using QF-approved encodings.
19. Write section directory, footer, postscript, and CRCs.
20. Optionally write QFM/QFX companion artifacts.
51.2 Statistics Policy
Converters SHOULD recompute QF statistics from decoded logical values.
...
 
52. Nested Columns
QF-T uses offset-based nested layouts.
52.1 List
List<T>:
  parent null bitmap
...
⸻
 
55. QF-O Object Temporal Profile
QF-O is an optional object-temporal profile.
...
⸻
 
57. QF-O Temporal Segment Index
...
- For Harbor QF-H/QF-O use, producer_scope_kind SHOULD be Tenant and producer_scope_id SHOULD be the Harbor tenant UUID.
...
Staging-only placeholders MUST NOT appear in QF.
59.2 Object Record Reference
struct QfRecordRefV1 {
    segment_id: u32,
    row_index: u32,
...
⸻
 
60. QF-O Reconstruction Self-Containment
QF-O v1 files MUST be reconstruction self-contained.
...
⸻
 
61. QF-O Property Columns
Object property columns use the same physical and encoded-array machinery as QF-T.
...
- QFX and QFM SHOULD reference QF files by cryptographic digest.
...
QF writers SHOULD organise files for:
- tail bootstrap,
...
⸻
 
68. QFX Accelerator Sidecar
QFX is an optional sidecar containing rebuildable acceleration metadata.
QFX final bytes:
[postscript bytes]
[postscript_version: u16]
[postscript_len: u16]
...
Rules:
- QFX MUST be ignored if referenced file digest does not match.
- QFX MUST be ignored if referenced file_id does not match.
- QFX MUST NOT change query semantics.
- QFX acceleration failures MUST fall back to QF.
 
⸻
 
69. QFM Dataset Manifest
QFM is an optional multi-file dataset manifest.
QFM final bytes:
[postscript bytes]
[postscript_version: u16]
[postscript_len: u16]
...
- object-store hints.
Rules:
- QFM MUST be ignored if stale.
- QFM MUST NOT change QF semantics.
- Query planners MAY use QFM to prune files before opening QF footers.
 
⸻
 
70. Profile Capability Matrix
A public QF implementation SHOULD declare which profile tier it supports.
Feature	QF-Core Reader	QF-T Scan Reader	QF-A Archive Reader	QF-E Reader	QF-H Harbor Reader
...
QFX sidecars	Optional	Optional	Optional	Optional	Optional
QFM manifests	Optional	Optional	Recommended	Optional	Recommended
QF-O object profile	Optional	Optional	Optional	Optional	Recommended for Harbor
QF-H Harbor mount profile	Not required	Not required	Not required	Not required	Required
 
⸻
 
71. Writer Profiles
71.1 QF-Core Minimal Profile
MUST emit:
- valid header,
- valid postscript,
- valid footer,
...
- valid null bitmaps.
71.2 QF-T Minimal Table Profile
MUST emit:
- all QF-Core requirements,
- table catalog,
- table segment index,
- table segment data,
- column page indexes,
...
71.4 QF-A Archive Acceleration Profile
...
71.5 QF-E Engine Execution Profile
...
71.7 QF-O Object Checkpoint Profile
...
QF-E optional profile corrupt	Ignore profile
QF-E required profile corrupt	Reject if needed
QFX stale/corrupt	Ignore QFX
QFM stale/corrupt	Ignore QFM
...
Invalid prev_ref	Reject QF-O file
...
Writers MUST publish QF files by durable replace.
Required protocol:
...
6. Only after step 5 may the new QF file be considered durable.
Rules:
...
- QF files MUST NOT be modified in place.
 
⸻
 
75. Error Codes
Code	Meaning
QF_E_BAD_MAGIC	Missing or invalid magic.
QF_E_BAD_VERSION	Unsupported QF version.
QF_E_UNKNOWN_REQUIRED_FEATURE	Unknown required feature bit set.
QF_E_CHECKSUM_MISMATCH	Header, postscript, footer, section, segment, or page checksum mismatch.
QF_E_DIGEST_MISMATCH	Cryptographic digest mismatch.
QF_E_OFFSET_RANGE	Offset/length/count exceeds file bounds.
QF_E_ARITH_OVERFLOW	Offset/count/size arithmetic overflow.
QF_E_BAD_SECTION	Section malformed or invalid.
QF_E_BAD_SCHEMA	Catalog/schema malformed.
QF_E_BAD_LOGICAL_PHYSICAL_PAIR	Logical type incompatible with physical kind.
QF_E_DICT_MISS	FileCode missing from dictionary.
QF_E_BAD_FILECODE	FileCode outside dictionary range.
QF_E_BAD_NUMCODE	NumCode invalid for declared logical type.
QF_E_BAD_DOMAIN	ColumnDomain invalid.
QF_E_BAD_STATS	Statistics invalid or unsafe.
QF_E_BAD_INDEX	Optional index invalid or corrupt.
QF_E_BAD_EXTENSION	Extension invalid or required extension unsupported.
QF_E_BAD_ENGINE_PROFILE	Engine profile invalid or unsupported when required.
QF_E_EXECUTION_CODE_MAP	Engine-local code mapping failed.
QF_E_HARBOR_MOUNT_LEASE	Harbor code lease resolution failed.
QF_E_REF_INVALID	QF-O prev_ref invalid.
QF_E_NOT_SELF_CONTAINED	QF-O chain lacks baseline/snapshot/full chain.
QF_E_SEGMENT_CORRUPT	Segment structure invalid.
QF_E_PAGE_CORRUPT	Page structure invalid.
QF_E_REDACTION_POLICY	Redacted value cannot be surfaced under current policy.
QF_E_SIDECAR_STALE	QFX/QFM sidecar does not match referenced QF.
 
⸻
 
76. Compatibility
76.1 Versioning
QF v1 readers support:
version_major = 1
Rules:
...
A conforming QF-Core reader MUST:
...
A conforming QF-T reader MUST additionally:
- parse table catalog,
...
A conforming QF-A reader SHOULD additionally:
- use lookup indexes when valid,
- use aggregate synopses when exact and applicable,
- use QFM for file pruning,
- use QFX when valid and beneficial,
- ignore stale or corrupt acceleration artifacts.
A conforming QF-E reader MUST additionally:
...
- never treat ExecutionCodes as QF logical truth.
A conforming QF-H reader MUST additionally:
...
A conforming QF-O reader MUST additionally:
...
An open QF release SHOULD include:
1. Reference reader.
2. Reference writer.
3. qf-validate CLI.
4. qf-inspect CLI.
5. qf-convert-parquet CLI.
...
- Parquet-to-QF conversion cost,
- QF file-size overhead,
- QFX/QFM acceleration impact,
...
The public QF project SHOULD ship the following utilities and artifacts.
79.1 Reference Libraries
qf-core:
  Format primitives, checksums, section directory, dictionary,
  encoded arrays, validation, collation, extension registry.

qf-reader:
  Read QF-Core and QF-T files.

qf-writer:
  Write QF-Core and QF-T files.

qf-arrow:
  Export QF data as Arrow arrays / record batches.

qf-engine:
  QF-E engine execution profile helpers.

qf-harbor:
  QF-H Harbor mount profile implementation.

qf-convert:
  Conversion library for Parquet/CSV/Arrow/ORC -> QF-T.
79.2 CLI Tools
qf-validate:
  Validate structure, CRCs, digests, schema, dictionaries,
  sections, indexes, profiles, extensions, and conformance.

qf-inspect:
  Print human-readable file layout, sections, catalog, stats,
  dictionary summaries, execution profiles, and index summaries.

qf-dump:
  Dump selected rows, columns, pages, morsels, dictionary values,
  or encoded array structures.

qf-convert-parquet:
  Convert Parquet files to QF-T.

qf-convert-arrow:
  Convert Arrow IPC/Feather/RecordBatch streams to QF-T.

qf-convert-orc:
  Convert ORC files to QF-T.

qf-bench:
  Run standard benchmarks against QF, Parquet, ORC, and optionally
  other formats.

qf-build-qfm:
  Build or refresh QFM dataset manifest.

qf-build-qfx:
  Build optional QFX accelerator sidecar.

qf-verify-digest:
  Verify cryptographic digests and Merkle roots.

qf-fuzz:
  Run corpus and property-based fuzz tests.

qf-canonicalise:
  Verify canonical value encodings, collation ordering,
  domain-rank construction, and trust input canonicalisation.

qf-profile:
  Inspect or generate QF-E engine profile metadata.

qf-arrow-export:
  Export QF-T tables to Arrow-compatible batches.
79.3 Engine Integrations
Recommended initial integrations:
Arrow:
  QF -> Arrow arrays and record batches.

DataFusion:
  QF TableProvider.

DuckDB:
  QF scan extension / table function.

Polars:
  QF scan/read support.

Python:
  qf.read_table(), qf.scan(), qf.to_arrow(), qf.to_polars().

Rust:
  qf-core, qf-io, qf-arrow, qf-datafusion, qf-engine.

Harbor:
  QF-H direct leased-code mount support.
79.4 Dataset and Benchmark Corpus
Recommended corpora:
synthetic-numeric:
  numeric full scan and range predicates.

synthetic-categorical:
  low/medium-cardinality FileCode workloads.

synthetic-wide:
  hundreds/thousands of columns with small projections.

synthetic-point:
  lookup-heavy high-cardinality IDs.

synthetic-composite:
  multi-column predicates and composite pruning.

synthetic-archive:
  multi-file object-store-style dataset with QFM.

parquet-tpch:
  converted TPC-H-style tables.

parquet-tpcds:
  converted TPC-DS-style tables.

parquet-medical-operational:
  categorical, temporal, event, and object-history style data.

negative-corrupt:
  malformed sections, invalid CRCs, bad offsets, invalid FileCodes.

canonicalisation:
  UTF-8, decimal, timestamp, UUID, NaN, null, map/list/struct cases.

engine-profile:
  FileCode -> ExecutionCode mapping tests for generic, Arrow, and Harbor profiles.
...
QF v1 chooses:
Neutral public name:
  Quay Format, with Harbor represented as QF-H profile and origin influence.

File-local FileCodes:
  over persisted engine-owned codes.

ExecutionCode abstraction:
  so non-Harbor engines can map FileCodes into their own runtime representations.

QF-E universal engine execution profile:
  over making Harbor-specific mount behaviour the generic extension mechanism.

QF-H Harbor profile:
  Harbor leased-code execution as one registered QF-E implementation.

Scope descriptors:
  over hard-coded tenant fields in the universal core.

Morsel-aligned pages:
  over generic row-group-only scans.

Encoded arrays:
  over flat codec-only compression.

Column domains:
  over raw FileCode min/max.

Predicate proof outcomes:
  over skip-only pruning.

Exact sets, blooms, lookup indexes, and aggregate synopses:
  over statistics-only acceleration.

Composite zone indexes:
  over single-column-only pruning.

QFX sidecars:
  over mutable in-file workload indexes.

QFM manifests:
  over opening every archive file for planning.

Extension registry:
  so custom logical types, indexes, synopses, encodings, and engine profiles
  are safe, discoverable, and either ignorable or required.

Arrow interop:
  so QF-T is useful without Harbor.

Lakehouse compatibility:
  so QF files can live inside existing catalog/table ecosystems.

Binary section directories:
  over JSON-authoritative metadata.

Digest manifests:
  over CRC-only archive integrity.

Self-contained object reconstruction:
  over mandatory cross-file prev_ref.

WORM durable replace:
  over in-place mutation.
The final shape is:
QF-Core:
  immutable binary foundation.

QF-T:
  engine-neutral table scan format.

QF-A:
  queryable archive acceleration profile.

QF-E:
  universal engine execution/mount profile.

QF-H:
  Harbor leased-code implementation of QF-E.

QF-O:
  object-temporal profile.

QFX:
  optional rebuildable accelerator sidecar.

QFM:
  optional multi-file dataset manifest.
This gives Quay Format a neutral public identity, a strict portable decode path, rich queryable archive acceleration, and a universal execution-profile mechanism while preserving the Harbor-native fast path that inspired the design.
