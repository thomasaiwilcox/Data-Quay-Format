...
use cove_core::{durable, utility::build_covm_artifact};

fn main() -> ExitCode {
    match run(env::args().skip(1).collect()) {
        Ok(()) => ExitCode::SUCCESS,
        Err(message) => {
            eprintln!("cove-build-covm: {message}");
            ExitCode::FAILURE
        }
    }
}
...
fn run(args: Vec<String>) -> Result<(), String> {
    if args.iter().any(|arg| arg == "-h" || arg == "--help") || args.len() < 2 {
        eprintln!("usage: cove-build-covm <output.covm> <input.cove>...");
        return Ok(());
    }
    let output = PathBuf::from(&args[0]);
    let inputs = args[1..].iter().map(PathBuf::from).collect::<Vec<_>>();
    let (bytes, report) = build_covm_artifact(&output, &inputs).map_err(|err| err.to_string())?;
