//! Cove Format (COVE) v1.0 — File header (`CoveHeaderV1`).
//!
//! Corresponds to Section 10 of the COVE v1.0 specification.
//!
//! The header is a fixed 128-byte structure at offset 0 of every COVE file.
...
/// Parsed COVE v1 file header.
///
/// Corresponds to `CoveHeaderV1` in Section 10 of the specification.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CoveHeaderV1 {
    /// Magic bytes — must be `b"COV1"`.
    pub magic: [u8; 4],
    /// Fixed at 128 for v1.
    pub header_len: u16,
...
impl CoveHeaderV1 {
    ...
    /// Parse and validate a COVE header from a 128-byte buffer.
    ///
...
mod tests {
    ...
    fn unknown_required_feature_rejected() {
        ...
        assert_eq!(
            CoveHeaderV1::parse(&bytes),
            Err(CoveError::UnknownRequiredFeature(0x0000_0001_0000_0000))
        );
    }
